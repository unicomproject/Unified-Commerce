...
public sealed class PlatformRoleRepositoryTests
{
    ...
    public async Task GetRolesAsync_ReadsSeededSuperAdministratorRole()
    {
        ...
        Assert.Equal(37, superAdmin.PermissionCount);
    ...
    [Fact]
    public async Task GetActiveBusinessPermissionIdMapAsync_ReturnsThirtySevenBusinessPermissions()
    {
        await using var dbContext = CreateDbContext();
        await PlatformAdminPermissionSeedApplicator.ApplyAsync(dbContext, Now);
        ...
        Assert.Equal(37, permissionMap.Count);
        Assert.Equal(
