...
public sealed class CollectionsController : ControllerBase
{
    ...
    public CollectionsController(ICollectionService collectionService, ITenantRequestContextFactory tenantRequestContextFactory)
    {
    ...
    public async Task<IActionResult> Create([FromBody] CollectionCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await _collectionService.CreateAsync(context, request, cancellationToken);
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("collection.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _collectionService.ListAsync(context, pageNumber, pageSize, search, cancellationToken);
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("collection.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _collectionService.GetByIdAsync(context, id, cancellationToken);
        return result.IsSuccess && result.Value is not null ? Ok(result.Value) : ToErrorResult(result.Error);
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status409Conflict)]
    public async Task<IActionResult> Update(Guid id, [FromBody] CollectionUpdateRequest request, CancellationToken cancellationToken)
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("collection.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _collectionService.UpdateAsync(context, id, request, cancellationToken);
        return result.IsSuccess && result.Value is not null ? Ok(result.Value) : ToErrorResult(result.Error);
    }
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("collection.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _collectionService.DeleteAsync(context, id, cancellationToken);
