...
public sealed class CategoriesController : ControllerBase
{
    ...
    public CategoriesController(ICategoryService categoryService, ITenantRequestContextFactory tenantRequestContextFactory)
    {
    ...
    public async Task<IActionResult> Create([FromBody] CategoryCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await _categoryService.CreateAsync(context, request, cancellationToken);
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("category.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _categoryService.GetByIdAsync(context, id, cancellationToken);
        return result.IsSuccess && result.Value is not null ? Ok(result.Value) : ToErrorResult(result.Error);
    }
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("category.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _categoryService.UpdateAsync(context, id, request, cancellationToken);
        return result.IsSuccess && result.Value is not null ? Ok(result.Value) : ToErrorResult(result.Error);
    }
    ...
    {
        if (!_tenantRequestContextFactory.TryCreate(User, out var context)) return Unauthorized(CreateError(new ApplicationError("category.invalid_tenant_context", "Invalid tenant context.")));
        var result = await _categoryService.DeleteAsync(context, id, cancellationToken);
