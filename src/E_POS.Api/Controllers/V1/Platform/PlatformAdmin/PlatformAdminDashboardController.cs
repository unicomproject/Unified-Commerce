...
[Route("api/v1/platform-admin/dashboard")]
public sealed class PlatformAdminDashboardController : ControllerBase
{
    private readonly IPlatformDashboardService _dashboardService;

    public PlatformAdminDashboardController(IPlatformDashboardService dashboardService)
    {
        _dashboardService = dashboardService;
    }

    [HttpGet]
    [ProducesResponseType(typeof(LegacyApiResponse<PlatformDashboardResponse>), StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> GetDashboard(CancellationToken cancellationToken)
    {
        ...
        var result = await _dashboardService.GetDashboardAsync(platformUserId, cancellationToken);

        if (result.IsSuccess && result.Value is not null)
        {
            return Ok(LegacyApiResponse<PlatformDashboardResponse>.Ok(
