...
namespace E_POS.Application.Modules.Tenant.Inventory.Shared.Validators;
...
public sealed class InventoryRequestValidator : IInventoryRequestValidator
{
    ...
    public ApplicationError? ValidateOpeningStock(OpeningStockRequest request)
    {
        var fieldErrors = new List<ApplicationFieldError>();
        ...
        if (request.Items is null || request.Items.Count == 0)
            fieldErrors.Add(new ApplicationFieldError("items", "At least one item is required for opening stock."));
        else if (request.Items.Count > 200)
            fieldErrors.Add(new ApplicationFieldError("items", "Cannot process more than 200 items in a single request."));
        ...
        if (request.Items != null)
        {
            for (var i = 0; i < request.Items.Count; i++)
            {
                ...
                if (item.Quantity <= 0)
                    fieldErrors.Add(new ApplicationFieldError($"items[{i}].quantity", "Opening stock quantity must be greater than zero."));
