...
public sealed class TillRequestValidator : ITillRequestValidator
{
    ...
    {
        return ValidateWriteRequest(request.OutletId, request.Name, request.Status);
    }
    ...
    private static ApplicationError? ValidateWriteRequest(Guid outletId, string name, string status)
    {
