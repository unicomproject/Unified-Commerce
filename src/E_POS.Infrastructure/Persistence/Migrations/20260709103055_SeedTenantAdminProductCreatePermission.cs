...
{
    /// <inheritdoc />
    public partial class SeedTenantAdminProductCreatePermission : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                FROM (
                    VALUES
                        ('77777777-0051-4000-8000-000000000001'::uuid, 'tenant.products.create', 'create', 'Create tenant products')
                ) AS seed(id, permission_code, action_type, description)
                CROSS JOIN (
                    SELECT module_id, feature_id
                    FROM permission_definitions
                    WHERE permission_code = 'catalog.products.create'
                    LIMIT 1
                ) AS product_template
                WHERE NOT EXISTS (
                    SELECT 1
                ...
                SELECT
                    ...
                    'Tenant admin product create permission seed.',
                    now()
                FROM (
                    VALUES
                        ('TENANT_ADMIN', 'tenant.products.create'),
                        ('STORE_MANAGER', 'tenant.products.create'),
                        ('INVENTORY_MANAGER', 'tenant.products.create')
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                WHERE tenant_role_permissions.permission_id = permission_definitions.id
                  AND permission_definitions.permission_code = 'tenant.products.create';

                DELETE FROM permission_definitions
                WHERE permission_code = 'tenant.products.create';
                """);
        }
    }
}
