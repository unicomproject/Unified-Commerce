...
{
    /// <inheritdoc />
    public partial class SeedTenantStockOpeningPermission : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                SELECT
                    ...
                    'Tenant admin opening stock permission seed.',
                    now()
                FROM (
                    VALUES
                        ('TENANT_ADMIN', 'tenant.stock.opening'),
                        ('STORE_MANAGER', 'tenant.stock.opening'),
                        ('INVENTORY_MANAGER', 'tenant.stock.opening')
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                WHERE tenant_role_permissions.permission_id = permission_definitions.id
                  AND permission_definitions.permission_code = 'tenant.stock.opening';

                DELETE FROM permission_definitions
                WHERE permission_code = 'tenant.stock.opening';
                """);
        }
    }
}
