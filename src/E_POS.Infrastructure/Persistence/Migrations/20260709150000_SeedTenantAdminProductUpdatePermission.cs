...
{
    /// <inheritdoc />
    public partial class SeedTenantAdminProductUpdatePermission : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                FROM (
                    VALUES
                        ('77777777-0053-4000-8000-000000000001'::uuid, 'tenant.products.update', 'update', 'Update tenant products')
                ) AS seed(id, permission_code, action_type, description)
                CROSS JOIN (
                    SELECT module_id, feature_id
                    FROM permission_definitions
                    WHERE permission_code = 'catalog.products.update'
                    LIMIT 1
                ) AS product_template
                WHERE NOT EXISTS (
                    SELECT 1
                ...
                SELECT
                    ...
                    'Tenant admin product update permission seed.',
                    now()
                FROM (
                    VALUES
                        ('TENANT_ADMIN', 'tenant.products.update'),
                        ('STORE_MANAGER', 'tenant.products.update'),
                        ('INVENTORY_MANAGER', 'tenant.products.update')
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                WHERE tenant_role_permissions.permission_id = permission_definitions.id
                  AND permission_definitions.permission_code = 'tenant.products.update';

                DELETE FROM permission_definitions
                WHERE permission_code = 'tenant.products.update';
                """);
        }
    }
}
