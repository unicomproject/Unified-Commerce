...
public sealed class DiscountPolicyConditionConfiguration : IEntityTypeConfiguration<DiscountPolicyCondition>
{
    public void Configure(EntityTypeBuilder<DiscountPolicyCondition> builder)
    {
        ...
        builder.HasOne<Tenant>().WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_conditions_tenant_id_tenants");
        builder.HasOne<DiscountPolicy>().WithMany().HasForeignKey(x => x.DiscountPolicyId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_conditions_discount_policy_id_discount_policies");
        builder.ToTable(t =>
        {
