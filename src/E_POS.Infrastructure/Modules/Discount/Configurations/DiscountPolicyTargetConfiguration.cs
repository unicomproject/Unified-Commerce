...
public sealed class DiscountPolicyTargetConfiguration : IEntityTypeConfiguration<DiscountPolicyTarget>
{
    public void Configure(EntityTypeBuilder<DiscountPolicyTarget> builder)
    {
        ...
        builder.HasOne<DiscountPolicy>().WithMany().HasForeignKey(x => x.DiscountPolicyId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_discount_policy_id_discount_policies");
        builder.HasOne<Product>().WithMany().HasForeignKey(x => x.ProductId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_product_id_products");
        builder.HasOne<ProductVariant>().WithMany().HasForeignKey(x => x.ProductVariantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_product_variant_id_product_variants");
        builder.HasOne<Category>().WithMany().HasForeignKey(x => x.CategoryId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_category_id_categories");
        builder.HasOne<Brand>().WithMany().HasForeignKey(x => x.BrandId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_brand_id_brands");
        builder.HasOne<Collection>().WithMany().HasForeignKey(x => x.CollectionId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_targets_collection_id_collections");
        builder.ToTable(t =>
        {
