...
public sealed class ProductInventorySettingConfiguration : IEntityTypeConfiguration<ProductInventorySetting>
{
    public void Configure(EntityTypeBuilder<ProductInventorySetting> builder)
    {
        ...
        builder.HasOne<Product>().WithMany().HasForeignKey(x => x.ProductId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_inventory_settings_product_id_products");
        builder.HasOne<ProductVariant>().WithMany().HasForeignKey(x => x.ProductVariantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_inventory_settings_product_variant_id_product_variants");
        ...
        builder.ToTable(t => { t.HasCheckConstraint("ck_product_inventory_settings_expiry_requires_batch", "requires_expiry_tracking = false OR requires_batch_tracking = true"); t.HasCheckConstraint("ck_product_inventory_settings_batch_requires_stock", "requires_batch_tracking = false OR is_stock_tracked = true"); t.HasCheckConstraint("ck_product_inventory_settings_serial_requires_stock", "requires_serial_tracking = false OR is_stock_tracked = true"); });
    }
}