...
public sealed class ProductBatchConfiguration : IEntityTypeConfiguration<ProductBatch>
{
    public void Configure(EntityTypeBuilder<ProductBatch> builder)
    {
        ...
        builder.HasOne<Tenant>().WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_batches_tenant_id_tenants");
        builder.HasOne<Product>().WithMany().HasForeignKey(x => x.ProductId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_batches_product_id_products");
        builder.HasOne<ProductVariant>().WithMany().HasForeignKey(x => x.ProductVariantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_batches_product_variant_id_product_variants");
        ...
        builder.ToTable(t => t.HasCheckConstraint("ck_product_batches_expiry_date", "expiry_date IS NULL OR manufactured_at IS NULL OR expiry_date >= manufactured_at"));
    }
}