...
public sealed class SalesOrderAddressConfiguration : IEntityTypeConfiguration<SalesOrderAddress>
{
    public void Configure(EntityTypeBuilder<SalesOrderAddress> builder)
    {
        ...
        builder.HasOne<E_POS.Domain.Modules.Tenant.TenantFoundation.Entities.Tenant>().WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_sales_order_addresses_tenant_id_tenants");
