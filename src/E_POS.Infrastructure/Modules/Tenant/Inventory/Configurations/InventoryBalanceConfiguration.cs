...
public sealed class InventoryBalanceConfiguration : IEntityTypeConfiguration<InventoryBalance>
{
    public void Configure(EntityTypeBuilder<InventoryBalance> builder)
    {
        ...
        builder.Property(x => x.RowVersion).HasColumnName("row_version").HasDefaultValue(0L).IsRequired().IsConcurrencyToken();
        ...
        builder.HasOne<InventoryLocation>().WithMany().HasForeignKey(x => new { x.TenantId, x.InventoryLocationId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_inventory_balances_inventory_location_id_inventory_locations");
        builder.HasOne<Product>().WithMany().HasForeignKey(x => new { x.TenantId, x.ProductId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_inventory_balances_product_id_products");
        builder.HasOne<ProductVariant>().WithMany().HasForeignKey(x => new { x.TenantId, x.ProductVariantId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_inventory_balances_product_variant_id_product_variants");
        builder.HasOne<ProductBatch>().WithMany().HasForeignKey(x => new { x.TenantId, x.ProductBatchId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_inventory_balances_product_batch_id_product_batches");
        ...
        builder.HasIndex(x => new { x.TenantId, x.Id }).IsUnique().HasDatabaseName("uq_inventory_balances_tenant_id_id");
        ...
        builder.ToTable(t =>
        {
            ...
            t.HasCheckConstraint("ck_inventory_balances_row_version", "row_version >= 0");
        });
    }
}
