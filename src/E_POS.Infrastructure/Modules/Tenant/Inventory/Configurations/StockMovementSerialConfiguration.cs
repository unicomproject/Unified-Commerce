...
public sealed class StockMovementSerialConfiguration : IEntityTypeConfiguration<StockMovementSerial>
{
    public void Configure(EntityTypeBuilder<StockMovementSerial> builder)
    {
        ...
        builder.HasOne<E_POS.Domain.Modules.Tenant.TenantFoundation.Entities.Tenant>().WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_stock_movement_serials_tenant_id_tenants");
