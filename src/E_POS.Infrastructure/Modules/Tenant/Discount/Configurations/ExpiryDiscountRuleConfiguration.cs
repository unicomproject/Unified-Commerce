...
public sealed class ExpiryDiscountRuleConfiguration : IEntityTypeConfiguration<ExpiryDiscountRule>
{
    public void Configure(EntityTypeBuilder<ExpiryDiscountRule> builder)
    {
        ...
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.CreatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rules_created_by_tenant_user_id_tenant_users");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.UpdatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rules_updated_by_tenant_user_id_tenant_users");
