...
public sealed class DiscountPolicyChannelConfiguration : IEntityTypeConfiguration<DiscountPolicyChannel>
{
    public void Configure(EntityTypeBuilder<DiscountPolicyChannel> builder)
    {
        ...
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.CreatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_channels_created_by_tenant_user_id_tenant_users");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.UpdatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_discount_policy_channels_updated_by_tenant_user_id_tenant_users");
