...
public sealed class ExpiryDiscountRuleTierConfiguration : IEntityTypeConfiguration<ExpiryDiscountRuleTier>
{
    public void Configure(EntityTypeBuilder<ExpiryDiscountRuleTier> builder)
    {
        ...
        builder.HasOne<E_POS.Domain.Modules.Tenant.TenantFoundation.Entities.Tenant>().WithMany().HasForeignKey(x => x.TenantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_tenant_id_tenants");
        
        builder.HasOne<ExpiryDiscountRule>().WithMany().HasForeignKey(x => new { x.TenantId, x.ExpiryDiscountRuleId }).HasPrincipalKey(x => new { x.TenantId, x.Id }).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_expiry_discount_rule_id_expiry_discount_rules");
        
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.CreatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_created_by_tenant_user_id_tenant_users");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.UpdatedByTenantUserId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_updated_by_tenant_user_id_tenant_users");

        builder.HasIndex(x => new { x.TenantId, x.Id }).IsUnique().HasDatabaseName("uq_expiry_discount_rule_tiers_tenant_id_id");
        ...
        builder.ToTable(t =>
        {
            ...
            t.HasCheckConstraint("ck_expiry_discount_rule_tiers_status", "status IN ('ACTIVE', 'INACTIVE', 'DELETED')");
        });
    }
}
