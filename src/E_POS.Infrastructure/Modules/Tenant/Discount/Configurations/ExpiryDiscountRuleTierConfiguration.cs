...
public sealed class ExpiryDiscountRuleTierConfiguration : IEntityTypeConfiguration<ExpiryDiscountRuleTier>
{
    public void Configure(EntityTypeBuilder<ExpiryDiscountRuleTier> builder)
    {
        ...
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.CreatedBy).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_created_by_tenant_user_id_tenant_users");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.UpdatedBy).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_expiry_discount_rule_tiers_updated_by_tenant_user_id_tenant_users");
