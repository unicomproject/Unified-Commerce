...
public sealed class ProductTaxAssignmentConfiguration : IEntityTypeConfiguration<ProductTaxAssignment>
{
    public void Configure(EntityTypeBuilder<ProductTaxAssignment> builder)
    {
        ...
        builder.HasOne<Product>().WithMany().HasForeignKey(x => x.ProductId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_tax_assignments_product_id_products");
        builder.HasOne<ProductVariant>().WithMany().HasForeignKey(x => x.ProductVariantId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_tax_assignments_product_variant_id_product_variants");
        builder.HasOne<TaxClass>().WithMany().HasForeignKey(x => x.TaxClassId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_tax_assignments_tax_class_id_tax_classes");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.CreatedBy).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_tax_assignments_created_by_tenant_user_id_tenant_users");
        builder.HasOne<TenantUser>().WithMany().HasForeignKey(x => x.UpdatedBy).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_product_tax_assignments_updated_by_tenant_user_id_tenant_users");
